...
description: Run a selective assisted FBS trading session that validates Telegram signals and strict technical setups, permits NO TRADE, renders the daily report, and publishes its summary to Telegram. Use immediately when the user says "nueva sesión", "nueva session", "iniciar sesión de trading", or otherwise asks for a new trading session, FBS signal review, Telegram delivery setup, or daily candidate report. Treat those short phrases as commands to run the workflow, not as greetings or requests for clarification. Never execute orders.
---
...
6. Summarize messages reviewed, candidates reviewed, valid candidates, the configured evaluated slots, backups, primary discard reasons, and Telegram delivery status.
...
- Treat Telegram as the first idea source. When it cannot supply the configured number of clean assets, market APIs may originate independently labeled technical setups from closed candles; never attribute those setups to an expert.
...
- Use USD 1,000 reference capital, USD 20 absolute safety ceiling per unique idea, and minimum R/R 1.6 unless local config overrides them. Apply the configured star-based risk tier and primary-basket cap before publication; only candidates at or above the actionable-star floor may be published.
...
- Include the exact entry, SL, TP, lot size, risk, and entry expiry. Entry expiry cancels only an unfilled pending order and is never a post-entry close instruction. Because the proxy may differ from FBS Bid/Ask, allow switching STOP/LIMIT within the same direction only when FBS requires it for the exact same entry; never change the levels or lot.

## Fallback opportunities

When fewer than the configured number of original candidates survive, run `scripts/derive_opportunities.py` through the main runner and evaluate the configured coverage layers:
...
- If seeded opportunities remain insufficient, scan configured FBS assets whose risk and lot size can be modeled. Publish only setup families allowed by config; strict mode rejects conditional-confirmation structures.
- Recalculate pending entry, order type, SL, TP, R/R, and setup-specific validity from closed-candle data before publishing. With structural-room validation enabled, the nearest confirmed 1h/4h swing must leave the minimum R/R and remain within the configured ATR distance.
...
- Produce at most one primary setup per asset and select no more than the configured primary count. Prefer selectivity and `NO TRADE` over filling a slot.
- Limit repeated directional USD exposure with the configured correlation cap. Discard opportunities below the actionable-star floor even if a slot remains empty.
- Fill a remaining primary slot with `NO TRADE` after the configured strict sources and modeled scan assets are evaluated. Record the scan count and missing confirmation; never invent levels or publish random filler.

## Output and safety

Write stable latest-snapshot artifacts under `sessions/YYYY-MM-DD/`: candidates JSON, metadata JSON, Markdown report, and Telegram delivery status. Also preserve every invocation under `sessions/YYYY-MM-DD/runs/<run-id>/` when run history is enabled. Keep the configured number of primary slots and backups.
